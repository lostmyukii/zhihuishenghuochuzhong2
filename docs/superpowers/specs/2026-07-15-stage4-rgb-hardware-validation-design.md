...
> 规格日期：2026-07-15
>
> 规格状态：方案A已获用户口头批准，书面规格待用户复核；尚未实施或烧录
>
...
- 主板当前运行本项目PIO固件`0.3.1`，GPIO13有源蜂鸣器已经完成上电静音、800ms短鸣、自动停止和真板验收。
...
### A：全环低亮青色800ms后自动熄灭（已选）

- 上电和复位时12颗灯珠全部熄灭，不运行启动灯效。
- 只在收到带ID的标准串口白名单命令后，将12颗灯珠设为青色。
- 全局亮度固定为`8 / 255`，点亮窗口固定为`800ms`。
- 到期后由非阻塞计时器自动清空并刷新灯环，不依赖第二条关闭命令。
- `rgb=off`可以在800ms窗口结束前立即熄灭。
- 本轮不把情境计划或安全覆盖中的`red`、`blue-red`等状态自动应用到实物。

该方案能用一次短暂现象确认GPIO46数据输出和整圈12颗灯珠，同时把USB供电负担和持续亮灯风险控制在较低水平。
...
3. 全局亮度上限固定为`8 / 255`；本轮任何路径都不得使用更高亮度或白色满亮输出。
...
pulse-cyan + 800ms到期 -> idle-off
pulse-cyan + rgb=off -> idle-off
```

- 使用`millis()`计算截止时间，不在主循环中使用`delay(800)`。
- 重复收到`rgb=cyan`时可以重新开始一次800ms窗口，但不能变成无限持续亮灯。
- `rgb=off`具有即时关闭优先级。
...
- `requestRgbTestPulse(nowMs)`：以固定低亮青色启动或刷新800ms窗口。
...
```json
{"type":"ack","id":"rgb-test-1","ok":true,"applied":{"rgbState":"cyan","rgbPulseMs":800,"rgbBrightness":8,"rgbPixels":12}}
```
...
`rgbState`只在GPIO46实际处于800ms青色输出窗口时为`cyan`，自动关闭后必须恢复为`off`。Dashboard不得从`actuatorTargets.rgbState`推断灯环真实亮起。
...
1. GPIO46、12颗灯珠、`NEO_GRB + NEO_KHZ800`和亮度`8 / 255`固定不变。
2. RGB独立开关开启、候选固件验证值为`false`，风扇、舵机和继电器开关保持关闭。
3. 灯环初始化后立即执行全灭刷新，不产生启动灯效。
4. GPIO11、GPIO9和GPIO12没有初始化或写入。
5. `rgb=cyan`产生800ms窗口并返回同ID成功`ack`。
6. 到期后自动全灭，且实现不包含阻塞式`delay(800)`。
7. `rgb=off`可以立即熄灭。
...
RGB计时和状态逻辑应尽量使用可在本机C++测试中注入假的`nowMs`的纯逻辑控制器；不通过真实等待800ms验证核心状态机。
...
5. 候选版本使用`0.3.2-rc1`，只写入PIO应用区`0x10000`；不擦除Flash，不改写bootloader、分区表或NVS。
6. 对写入后的实际应用段执行摘要校验。
7. 复位并观察：RGB必须保持全灭、蜂鸣器保持静音，串口持续输出正确`hello/telemetry`。
8. 发送一次`rgb=cyan`，用户确认12颗灯珠整圈以低亮青色亮起，并在约800ms后自动熄灭。
...
- 标准命令使整圈12颗灯珠呈低亮青色，并在约800ms后自动熄灭。
- 关闭命令可以即时清空灯环。
...
> GPIO46的12颗RGB灯环已完成上电全灭、标准命令触发、`8 / 255`低亮青色800ms显示、自动熄灭、关闭命令和串口真实状态验收。
