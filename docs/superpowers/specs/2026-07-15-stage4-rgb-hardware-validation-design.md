...
> 规格日期：2026-07-15
>
> 规格状态：RGB点灯诊断已完成并安全回退；用户现已批准A5——用已验证有源蜂鸣器检查GPIO46基础高低电平输出
>
> 实施边界：正式GPIO合同仍为RGB=GPIO46、蜂鸣器=GPIO13；A4只在临时候选固件中复用已验收GPIO13端口驱动灯环，蜂鸣器必须禁用，测试后恢复正式合同
...
- 主板当前运行候选固件`0.3.2-rc3`；GPIO13有源蜂鸣器已经完成上电静音、800ms短鸣、自动停止和真板验收。
...
- 第三轮`0.3.2-rc3`只驱动第1颗红灯`128 / 255`约5秒，回执、遥测和自动关闭正常；原灯环与更换后的第二个灯环均无肉眼可见输出。
- 复核原图后确认灯环端标记实际为字母`SI/SO`而非数字`S1/S0`：`SI`是输入，`SO`是级联输出。早期信号误接`SO`，该判断已纠正。
...
### A4：已验收GPIO13端口交叉诊断（已完成）

- 正式常量`PIN_RGB=46`保持不变，另设仅用于候选固件的`RGB_DIAGNOSTIC_PIN=13`。
- 候选版本为`0.3.2-rc4-pin13-diagnostic`，`BUZZER_ARMED=false`、`RGB_ARMED=true`；GPIO13不得同时初始化为蜂鸣器输出。
- 灯环必须使用`GND / VCC / SI`输入端，测试动作仍只点索引0红灯、亮度`128 / 255`、持续5000ms，不提高亮度或扩大灯珠数量。
- `hello.pins.rgb`继续报告正式端口46，同时在健康字段中明确报告临时诊断端口13，避免把交叉诊断误写成正式GPIO变更。
- 若GPIO13可见亮灯，说明灯环与当前供电/协议至少能工作，下一步检查GPIO46拓展板端口或信号链路；若仍不亮，则优先检查供电、线材、灯环协议或驱动兼容性，不能只归因于GPIO46。
- A4结束后必须把主板恢复为RGB未武装的正式安全固件；没有人工可见证据时不得设置`RGB_HARDWARE_VERIFIED=true`。

### A5：GPIO46有源蜂鸣器基础输出诊断（当前已批准）

- 正式合同仍为蜂鸣器GPIO13、RGB GPIO46；候选固件只把已验证的800ms蜂鸣脉冲临时路由到GPIO46，不永久交换两个模块。
- 候选版本使用`0.3.3-rc1-gpio46-buzzer-diagnostic`，必须明确报告实际测试输出引脚46；RGB保持未武装，GPIO13保持空置。
- 初始化顺序仍为先向GPIO46写`LOW`再设为`OUTPUT`，上电不得鸣叫；只有收到带ID的标准`buzzer=true`命令才输出`HIGH`约800ms，随后非阻塞恢复`LOW`。
- 测试期间只连接已验证有源蜂鸣器，RGB灯环、风扇、舵机和继电器不得连接或初始化。
- 若听到一次短鸣并自动停止，说明GPIO46拓展板端口至少能完成基础数字输出，RGB失败进一步收敛到NeoPixel波形/驱动兼容性；若仍静音，则GPIO46基础输出或拓展板链路未通过。
- 无论结果如何，都必须写回`0.3.2`安全固件并确认`RGB_ARMED=false`、GPIO46不再输出、蜂鸣器正式输出引脚恢复为GPIO13。
...
1. 正式合同固定使用`PIN_RGB = 46`、12颗灯珠、`NEO_GRB + NEO_KHZ800`；A4只允许候选固件通过独立常量把物理输出临时路由到GPIO13，不改变既定GPIO表。
...
6. A4以外的GPIO46候选保留GPIO13蜂鸣器的安全低电平和独立驱动；A4候选必须完全禁用蜂鸣器初始化和命令，不把RGB动作与蜂鸣器联动。
7. 如果灯环持续高亮、颜色异常、闪烁不止、主板反复重启、USB掉线或模块异常发热，立即拔下USB并停止测试。
8. A4出现可见红灯只证明GPIO13交叉路径下的单颗短时输出，不证明GPIO46、整圈灯珠、全部颜色通道、长期灯效或安全报警灯效已经验收。
9. A4期间蜂鸣器必须未武装，任何蜂鸣器命令都应返回`actuators_unarmed`；不得在GPIO13上同时运行蜂鸣器与NeoPixel驱动。
...
> 同一灯环、线材和`SI`接法在GPIO13可见点亮，证明灯环基本链路可工作；接回GPIO46时命令回执、遥测计时和自动关闭正常但无可见输出。GPIO46 RGB验收未通过，最终固件保持RGB未武装、未验收；下一步检查GPIO46基础输出或拓展板信号链路。
