...
阶段0至阶段5软件基线和`0.4.0`应用区烧录/静态串口验收已经完成。当前开发板运行`0.4.0`；本轮只验证启动保护、传感器、MQ2预热和默认安全状态，没有发送模式或执行器命令。
...
`0.4.0`应用区烧录、独立校验、5秒启动保护和30秒MQ2预热静态验收已完成。下一步只能在用户选择并再次授权后进行：其一是用Dashboard的Web Serial做只读页面闭环；其二是保持低压安全条件，逐项发送风扇、舵机、继电器、蜂鸣器和RGB命令并由用户观察实物。完成对应人工证据前不得把风扇、舵机、继电器或RGB写成已验收，也不得声称整屋物理联动已经通过。
